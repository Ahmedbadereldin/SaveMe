package com.serveme.framework.androidmodification;
...
/**
 * An adapter that exposes data from a series of {@link android.database.Cursor}s to an
 * {@link android.widget.ExpandableListView} widget. The top-level {@link android.database.Cursor} (that is
 * given in the constructor) exposes the groups, while subsequent {@link android.database.Cursor}
 * s returned from {@link #getChildrenCursor(android.database.Cursor)} expose children within a
...
public abstract class CursorTreeAdapter extends BaseExpandableListAdapter
    ...
    /**
     * Constructor. The adapter will call {@link android.database.Cursor#requery()} on the cursor
     ...
     * @param autoRequery If true the adapter will call {@link android.database.Cursor#requery()} on the
     *                    cursor whenever it changes so the most recent data is always
     *                    displayed.
     */
     ...
     *                      {@link #getChildrenCursor(android.database.Cursor)} (true), or to assume a call
     *                      to {@link #setChildrenCursor(int, android.database.Cursor)} will happen shortly
     *                      (false).
    ...
    synchronized MyCursorHelper getChildrenCursorHelper(int groupPosition,
        ...
        if (cursorHelper == null) {
            ...
            final Cursor cursor = getChildrenCursor(mGroupCursorHelper
                    .getCursor());
     ...
     *
     * @param groupCursor The cursor pointing to the group whose children cursor should
     *                    be returned
     ...
     *
     * @param cursor The Cursor to set for the group. If there is an existing
    ...
    /**
     * Sets the children Cursor for a particular group. If there is an existing
     * cursor it will be closed.
     * <p/>
     ...
     */
    public synchronized void setChildrenCursor(int groupPosition,
    ...
    public Cursor getChild(int groupPosition, int childPosition) {
        ...
        return getChildrenCursorHelper(groupPosition, true).moveTo(
                childPosition);
    }

    public long getChildId(int groupPosition, int childPosition) {
        return getChildrenCursorHelper(groupPosition, true)
                .getId(childPosition);
    }

    public int getChildrenCount(int groupPosition) {
        MyCursorHelper helper = getChildrenCursorHelper(groupPosition, true);
        return (mGroupCursorHelper.isValid() && helper != null) ? helper
                .getCount() : 0;
    }
     ...
     * @param isExpanded Whether the group is expanded.
     */
    protected abstract void bindGroupView(View view, Context context,
     ...
     * @param isLastChild Whether the child is the last child within its group.
     */
    protected abstract void bindChildView(View view, Context context,
     ...
     * cached cursors.
     *
     * @param releaseCursors Whether to release and deactivate any cached cursors.
     */
    ...
    /**
     * Helper class for Cursor management: <li>Data validity <li>Funneling the
     * content and data set observers from a Cursor to a single data set
     * observer for widgets <li>ID from the Cursor for use in adapter IDs <li>
